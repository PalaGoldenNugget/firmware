...
void drawBigDigit(int x, int y, int w, int h, char d, uint8_t c) {
  ...
  switch (d) {
    ...
    case '-':
      ...
      break;
    case ':': {
      int cxp = x + w/2, rr = max(2, t/2 + 1);
      fillCircle(cxp, y + hh - hh/3, rr, c);
      fillCircle(cxp, y + hh + hh/3, rr, c);
      break;
    }
    default: break;
...
// (heavy bitmap digit font removed — countdown uses built-in FreeSansBold18pt)
...
// Centered text that prefers `scale` (e.g. 2 = clean bold) but drops to a
// smaller scale if the line would exceed maxW — so the whole string is kept
// (no truncation) and never clips the panel edge. Returns the scale used.
int drawStrCFitScale(int cx, int y, int maxW, const char* s, int scale, uint8_t color) {
  while (scale > 1 && textW(s, scale) > maxW) scale--;
  drawStr(cx - textW(s, scale) / 2, y, s, scale, color);
  return scale;
}
